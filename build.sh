...
# 编译 sed
curl -fLO https://ftp.gnu.org/gnu/sed/sed-4.10.tar.gz
tar -zxf sed-4.10.tar.gz
cd sed-4.10
./configure --prefix=/opt/sed-4.10-ohos-arm64
make -j$(nproc)
make install
cd ..

# 进行代码签名
cd /opt/sed-4.10-ohos-arm64
...
cp sed-4.10/COPYING /opt/sed-4.10-ohos-arm64/
cp sed-4.10/AUTHORS /opt/sed-4.10-ohos-arm64/

# 打包最终产物
cp -r /opt/sed-4.10-ohos-arm64 ./
tar -zcf sed-4.10-ohos-arm64.tar.gz sed-4.10-ohos-arm64
